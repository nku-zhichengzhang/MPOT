...
class MOTMetrics(Metrics):
	def __init__(self, seqName = None):
		...
		# Evaluation metrics
		self.register(name = "SR", formatter='{:.2f}'.format, )
	...
	def compute_clearmot(self):
	    ...
	    if self.n_gt == 0:
	        ...
	        self.MOTA = (1 - (self.fn + self.fp + self.id_switches) / (3 * float(self.n_gt)) ) * 100.
	    ...
	    if self.n_gt != 0:
	        if self.id_switches == 0:
	            self.MOTAL = (1 - (self.fn + self.fp + self.id_switches)  / (3 * float(self.n_gt)) ) * 100.0
	        else:
	            self.MOTAL = (1 - (self.fn + self.fp + math.log10(self.id_switches)) /(3 * float(self.n_gt)) ) * 100.
	    ...
	    IDPrecision = self.IDTP / (self.IDTP + self.IDFP) if self.IDTP + self.IDFP!=0.0 else 0.0
	    IDRecall = self.IDTP / (self.IDTP + self.IDFN) if self.IDTP + self.IDFN!=0.0 else 0.0
	    self.IDF1 = 2*self.IDTP/(self.n_gt + self.n_tr) if self.n_gt + self.n_tr!=0.0 else 0.0
