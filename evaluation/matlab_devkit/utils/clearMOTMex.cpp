...
inline double corner_dis(double det[8], double gt[8])
{
    double dis = 0.0;
    for (int i = 0; i < 4; i++)
        dis += euclidean(det[2*i],det[2*i+1],gt[2*i],gt[2*i+1]);
	return dis/4;
}
		...
		if (t > 0)
		{
			...
			for (int k = 0; k < mappings.size(); k++)
			{
				...
				if (foundGtind != gtInd[t].end() && foundStind != stInd[t].end())
				{
                    ...
                    det[0] = gtMat[index(rowgt, 2, rowsGT)];
                    det[1] = gtMat[index(rowgt, 3, rowsGT)];
                    det[2] = gtMat[index(rowgt, 4, rowsGT)];
                    det[3] = gtMat[index(rowgt, 5, rowsGT)];
                    det[4] = gtMat[index(rowgt, 6, rowsGT)];
                    det[5] = gtMat[index(rowgt, 7, rowsGT)];
                    det[6] = gtMat[index(rowgt, 8, rowsGT)];
                    det[7] = gtMat[index(rowgt, 9, rowsGT)];

                    gt[0] = resMat[index(rowres, 2, rowsST)];
                    gt[1] = resMat[index(rowres, 3, rowsST)];
                    gt[2] = resMat[index(rowres, 4, rowsST)];
                    gt[3] = resMat[index(rowres, 5, rowsST)];
                    gt[4] = resMat[index(rowres, 6, rowsST)];
                    gt[5] = resMat[index(rowres, 7, rowsST)];
                    gt[6] = resMat[index(rowres, 8, rowsST)];
                    gt[7] = resMat[index(rowres, 9, rowsST)];
                    double dist = corner_dis(det,gt);
